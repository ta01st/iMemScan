...
class VMSearchCtrl: UITableViewController, UITextFieldDelegate {
    ...
    func setupViews() {
        ...
        let titles = ["全改", "选择", "数值搜索", "邻近搜索"]
        var items: [AnyHashable]? = []
        
        for i in 0..<titles.count {
            let button = UIButton(type: .system)
            
            if titles[i] == "全改" || titles[i] == "选择"  {
                button.frame = .init(x: 0, y: 0, width: 50, height: 25)
            }
            
            if titles[i] == "数值搜索" || titles[i] == "邻近搜索"  {
                button.frame = .init(x: 0, y: 0, width: 80, height: 25)
            }
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem(customView: {
            ...
            button.setTitle("清除", for: .highlighted)
            button.layer.cornerRadius = 12.5
            button.backgroundColor = .secondarySystemBackground
            button.tintColor = UIColor.dynamicColor(.gray, darkColor: UIColor.white)
    ...
    @objc func buttonAction(_ button: UIButton?) {
        ...
        if title == "清除结果" {
            ...
            if dataArray.count > 0 {
                button?.setTitle("确认", for: UIControl.State(rawValue: 0))
    ...
    func nearSearchAction() {
        ...
        if dataArray.count > 1000000 {
            UIAlertController.showAlert4("🙄", message: "求你点清除吧,都100万数据了还邻近搜索个鬼吖", btnTitle: "ok")
            return
        }
    ...
    func showAlert(_ title: String?) {
        ...
        let btn2 = JHUIAlertButtonConfig(title: "搜索", color: nil, font: nil, image: nil, handle: { [weak wk = self] in
            wk!.refreshAction()
        })
    ...
    func allModifyAction() {
        modifyType = 1
        showAlert("全部修改", holder: "", buttonTitle: "全改", indexPath: nil)
    }
    
    // MARK: - 批量修改
    
    func partModifyAction(_ button: UIButton?) {
        button?.setTitle("选择", for: UIControl.State(rawValue: 0))
        
        var titles: [AnyHashable]? = nil
        titles = ["全改", "选择", "数值搜索", "邻近搜索"]
        let buttons = [button1, button2, button3, button4]
        for i in 0..<(titles?.count ?? 0) {
            let btn = buttons[i]
            btn?.setTitle(titles?[i] as? String, for: UIControl.State(rawValue: 0))
        ...
        if flag {
            modifyType = 2
            showAlert("批量修改", holder: "", buttonTitle: "修改", indexPath: nil)
        }
    }
    ...
    @objc func longPressAction(_ gesture: UIGestureRecognizer?) {
        ...
        let title = "储存地址 " + (model.address)
        UIAlertController.showAlert_Longpress(title, message: nil, holder: "名称", buttonTitle: "储存", handler: { text in
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        modifyType = 0
        let model = dataArray[indexPath.row] as! MemModel
        let title = "修改 \(model.address)"
        showAlert(title, holder: model.value, buttonTitle: "修改", indexPath: indexPath)
    }
