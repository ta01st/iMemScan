...
extension AboutCtrl: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 1:
            cell?.textLabel?.text = "卸载应用"
            cell?.imageView?.image = UIImage(systemName: "clear")
            cell?.backgroundColor = .secondarySystemBackground
        case 2:
            cell?.textLabel?.text = "下载更新"
            cell?.imageView?.image = UIImage(systemName: "icloud.and.arrow.down")
            cell?.backgroundColor = .secondarySystemBackground
            
            let fliePath = Bundle.main.path(forResource: "iMemScan", ofType: nil)
...
//        }
    ...
    func remove_deb(_ tableView: UITableView?, index indexPath: IndexPath?) {
        ...
        let alertCtrl = UIAlertController(title: "卸载指令", message: "确定要卸载本应用吗？", preferredStyle: .actionSheet)
        
        alertCtrl.addAction(UIAlertAction(title: "确定卸载", style: .destructive, handler: { action in
