...
class VMRecordCtrl: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    ...
    func oneKeyOffset() {
        ...
        if dataArray.count == 0 {
            UIAlertController.showAlert4("error", message: "请先去添加保存地址", btnTitle: "ok")
            return
        }
        
        UIAlertController.showAlert_Longpress("储存列表重定基址", message: "规则 + / -  例如输入: +4", holder: "偏移量", buttonTitle: "偏移", handler: { [self] text in
    ...
    func allModify() {
        modifyType = 1
        showAlert("全部修改", holder: nil, buttonTitle: "修改", table: tableView, indexPath: nil)
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if (model?.recordName.count)! > 0 {
            ...
            showAlert(title, holder: model?.value, buttonTitle: "修改", table: tableView, indexPath: indexPath)
        } else {
            var title: String? = nil
            if let recordName = model?.recordName {
                title = "数值修改 \(recordName)"
            }
            showAlert(title, holder: model?.value, buttonTitle: "修改", table: tableView, indexPath: indexPath)
        }
    }
    ...
    @objc func longPressAction(_ gesture: UIGestureRecognizer?) {
        ...
        let model = dataArray[indexPath!.row] as! MemModel
        let alertCtrl = UIAlertController(title: nil, message: "管理 \((model as AnyObject).address ?? "")", preferredStyle: .actionSheet)
        
        alertCtrl.addAction(UIAlertAction(title: "调整基址", style: .default, handler: { [self] action in
            
            let title = "储存列表重定基址"
            UIAlertController.showAlert_Longpress(title, message: nil, holder: "偏移量", buttonTitle: "偏移", handler: { [self] text in
                let offset: String = text!
        ...
        alertCtrl.addAction(UIAlertAction(title: "修改名称", style: .default, handler: { [self] action in
            
            let title = "修改名称 \(model.address)"
            UIAlertController.showAlert_Longpress(title, message: nil, holder: "名称", buttonTitle: "修改", handler: { [self] text in
        ...
        alertCtrl.addAction(UIAlertAction(title: "查看内存", style: .default, handler: { [self] action in
            // 传送地址
        ...
        alertCtrl.addAction(UIAlertAction(title: "删除储存", style: .destructive, handler: { [self] action in
